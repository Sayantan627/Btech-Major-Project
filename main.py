...
while True:
    ...
    if ret:
        ...
        parking_lots = parking_availability(frame)
        ...
        if len(available_parking_lot) > 0:
            ...
            closest_parking_dist = distance_calc.calculate_distance(closest_parking, entrance)
            distance_meters = closest_parking_dist / 40
            estimated_time_sec = round(distance_meters / 2.5, 2)
        ...
        cv2.putText(img=frame,
                    text="Est. time to closest: {} sec".format(estimated_time_sec),
                    org=(10, 470),
                    fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=0.5,
                    color=(255, 0, 255),
